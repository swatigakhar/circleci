...
async function createMail(logData, reportUrl, mailTitle) {
    ...
    if (null == tc_passed || null == tc_failed || !envDetail || !appUrl || /*reportStatus != 200 ||*/ tc_total == 0) {
        ...
        subject = "CircleCI Report (github repo)" + "❌ " + argv.appType + " | " + argv.testEnv.toUpperCase() + " | " + folder[0] + " | Error in sending Mail";
        tc_status = "error";
        mailingList = errorMailingList;
    } else {
        ...
        if (tc_status == "passed")
            subject = "CircleCI Report: (github repo)" + "✔️ " + argv.appType + " | " + argv.testEnv.toUpperCase() + " | " + folder[0] + " | " + tc_status.toUpperCase();
        else
            subject = "CircleCI Report: (github repo)" + "❌ " + argv.appType + " | " + argv.testEnv.toUpperCase() + " | " + folder[0] + " | " + tc_status.toUpperCase();
